<template>
    <div class="ctool-video-format-convert">
        <Card class="ctool-page-option" padding="10px">
            <div class="ctool-video-format-convert-toolbar">
                <div class="ctool-video-format-convert-desc">视频格式转换（浏览器端 ffmpeg）</div>
                <Align>
            ...
            <div class="ctool-video-format-convert-options">
                <div class="ctool-video-format-convert-field">
                    <div class="ctool-video-format-convert-label">文件</div>
                    <input type="file" accept="video/*" @change="fileChangeHandle" />
                    <div class="ctool-video-format-convert-file-name" v-if="fileName">{{ fileName }}</div>
                </div>
                <div class="ctool-video-format-convert-field">
                    <div class="ctool-video-format-convert-label">目标格式</div>
                    <Select v-model="target" :options="formatOptions" />
                </div>
            </div>
            <div class="ctool-video-format-convert-status">{{ statusText }}</div>
        </Card>
        <HeightResize v-slot="{ height }" :append="['.ctool-page-option']" :reduce="5">
            <div class="ctool-video-format-convert-layout" :style="{ height: `${height}px` }">
                <Card class="ctool-video-format-convert-card" title="预览">
                    <div class="ctool-video-format-convert-preview">
                        <video v-if="outputUrl" controls :src="outputUrl" />
                        <div v-else class="ctool-video-format-convert-empty">暂无结果</div>
                    </div>
                </Card>
                <Card class="ctool-video-format-convert-card" title="下载">
                    <div class="ctool-video-format-convert-preview">
                        <a v-if="outputUrl" :href="outputUrl" :download="outputName">下载 {{ outputName }}</a>
                        <div v-else class="ctool-video-format-convert-empty">请先转换</div>
                    </div>
                </Card>
            </div>
        </HeightResize>
...
const formatOptions: FormatOption[] = [
    { label: "AVI", value: "avi", mime: "video/x-msvideo" },
    { label: "MP4", value: "mp4", mime: "video/mp4" },
    { label: "MPEG", value: "mpeg", mime: "video/mpeg" },
    { label: "OGG", value: "ogg", mime: "video/ogg" },
    { label: "3GP", value: "3gp", mime: "video/3gpp" },
    { label: "TS", value: "ts", mime: "video/mp2t" },
    { label: "WEBM", value: "webm", mime: "video/webm" },
    { label: "FLV", value: "flv", mime: "video/x-flv" },
    { label: "MOV", value: "mov", mime: "video/quicktime" },
];
...
const outputName = computed(() => {
    const base = (fileName.value || "video").replace(/\.[^.]+$/, "");
...
<style>
.ctool-video-format-convert {
    height: 100%;
}

.ctool-video-format-convert-toolbar {
    display: flex;
...
.ctool-video-format-convert-desc {
    font-size: .95rem;
}

.ctool-video-format-convert-options {
    display: grid;
...
.ctool-video-format-convert-field {
    min-width: 0;
}

.ctool-video-format-convert-label {
    margin-bottom: 4px;
    font-size: .8rem;
    color: var(--ctool-info-color);
}

.ctool-video-format-convert-file-name {
    margin-top: 4px;
    font-size: .8rem;
    color: var(--ctool-info-color);
}

.ctool-video-format-convert-status {
    margin-top: 10px;
    padding: 8px 10px;
...
.ctool-video-format-convert-layout {
    display: grid;
    grid-template-columns: repeat(2, minmax(0, 1fr));
    gap: 5px;
}

.ctool-video-format-convert-card,
.ctool-video-format-convert-preview {
    min-height: 0;
    height: 100%;
}

.ctool-video-format-convert-card .ctool-card-body {
    height: 100%;
}
...
.ctool-video-format-convert-preview video {
    width: 100%;
    max-height: 100%;
}

.ctool-video-format-convert-empty {
    color: var(--ctool-info-color);
    font-size: .9rem;
}
