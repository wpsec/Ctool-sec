...
## 2026-03-25 更新
- 本次新增/接入 **45 个工具**（按一级分类）：
- 图片加工（9）：`图片转 Base64`、`图片添加水印`、`图片添加圆角`、`图片去色`、`图片像素化`、`图片格式转换`、`图片压缩`、`图片 OCR 识别`、`多格切图`
- 二进制处理（4）：`Hex 编辑器`、`Pyc 转 Py`、`Pyc 转 Asm`、`文件格式识别`
- 编码解码（3）：`摩斯电码`、`JSFuck 编码`、`核心价值观编解码`
- 安全（5）：`资产分拣`、`ZIP 伪加密`、`XSS 向量生成器`、`Htpasswd`、`随机 USER-AGENT`
- 辅助开发（5）：`Git 指令速查`、`SQLite 在线浏览`、`CIDR 聚合工具`、`文件树生成`、`随机 IP 生成器`
- JSON 加工（1）：`JSON JavaScript 互转`
- 格式化（2）：`LESS CSS 互转`、`Markdown HTML 互转`
- 文字处理（3）：`Leet 转换器`、`Figlet`、`乱序文字生成器`
- 视频音频（5）：`音频格式转换`、`视频格式转换`、`视频提取音频`、`视频帧截图`、`视频转 GIF`
- 办公工具（5）：`南丁格尔玫瑰图`、`雷达图`、`折线图`、`手绘白板`、`倒计时`
- 其他（3）：`早中晚吃啥呢`、`随机邮箱生成`、`人生格子`
